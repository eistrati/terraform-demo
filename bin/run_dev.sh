...
recink run terraform -vv --include-modules="ec2_dev" \
  --custom-config="ec2_dev.terraform.plan:true\
,ec2_dev.terraform.apply:false\
,ec2_dev.terraform.save-show-output:tfshow.txt" || on_fail
recink run unit || on_fail
recink run terraform --include-modules="ec2_dev" \
  --custom-config="ec2_dev.terraform.plan:true\
,ec2_dev.terraform.apply:true" || on_fail
